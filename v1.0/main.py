...
class GeneradorHTMLPinyin:
    ...
    def __init__(self, hardware_info):
        """Inicializa el generador con configuración basada en hardware"""
        self.hardware_info = hardware_info
        self.template_cache = {}
        self.cache_traducciones_grupos = {}  # Cache para traducciones de grupos
    ...
    def generar_css_optimizado(self):
        """Genera CSS ultra optimizado según hardware"""
        ...
        <style>
            ...
            * {{
            ...
            .pinyin-row + .chinese-row {{
                margin-top: -1px; /* Eliminar separación entre pinyin y caracteres */
            }}
            ...
            /* Estilos para las filas de traducción individuales */
            .translation-row {{
    ...
    def generar_javascript_interactivo(self, contenido_completo=None):
        """Genera JavaScript simplificado solo para actualizar traducción"""
        
        return """
        <script>
            // Script simplificado - solo para funcionalidades básicas
    ...
    def generar_html_optimizado(self, datos_tabla, titulo="Pronunciación Pinyin", contenido_completo=None, idioma_destino='es'):
        ...
        # Generar filas de tabla optimizadas con orden vertical correcto
        # Orden deseado: PINYIN -> CARÁCTER -> TRADUCCIÓN (para cada segmento)
        filas_html = []
        i = 0
        ...
        while i < len(datos_tabla):
            ...
            # Verificar si es una fila separadora (vacía, con estrellas o con solo símbolos)
            if not fila_actual or all(not str(celda).strip() for celda in fila_actual) or \
               (len(fila_actual) == 1 and str(fila_actual[0]).strip() in ['⭐', '━━━', '┈┈┈']):
                # Fila separadora más compacta
                filas_html.append('<tr class="separator-row"><td colspan="100%" style="text-align: center; padding: 4px; color: #94a3b8; font-size: 14px;">⭐</td></tr>')
                i += 1
                continue
            ...
            if i + 1 < len(datos_tabla):
                ...
                for celda in fila_siguiente:
                    ...
                    if celda_str:
                        ...
                        # Recoger caracteres chinos para traducir
                        for char in celda_str:
                            if '\u4e00' <= char <= '\u9fff':
                                caracteres_para_traducir.append(char)
                i += 2  # Saltar ambas filas
            else:
                i += 1  # Solo una fila
            
            # Agregar filas completas horizontalmente (como antes, más eficiente)
            if fila_pinyin:
            ...
            if fila_caracteres:
                ...
                # Agregar fila de traducción para todo el grupo (más eficiente)
                if caracteres_para_traducir and len(caracteres_para_traducir) >= 3:  # Mínimo 3 caracteres para evitar sobrecarga
                    ...
                    if cache_key in cache_traducciones:
                        ...
                        try:
                            ...
                            if traduccion_grupo and traduccion_grupo.strip():
                                cache_traducciones[cache_key] = traduccion_grupo
                            ...
                            print(f"Error traduciendo grupo '{texto_grupo}': {e}")
                            traduccion_grupo = None
                    
                    # Solo agregar fila de traducción si la traducción fue exitosa
                    if traduccion_grupo and traduccion_grupo.strip():
                        # Agregar emoji según el idioma
                        emoji_idioma = "🇪🇸" if idioma_destino == 'es' else "🇺🇸"
                        nombre_idioma = "Español" if idioma_destino == 'es' else "English"
                        clase_idioma = "espanol" if idioma_destino == 'es' else "ingles"
                        
                        filas_html.append(f'''
                        <tr class="translation-row">
                            <td colspan="100%" class="translation-cell">
                                <div class="translation-content {clase_idioma}">
            ...
            # Separador más grande entre grupos completos
            if i < len(datos_tabla):
                filas_html.append('<tr style="height: 12px;"><td colspan="100%"></td></tr>')
        ...
        <body>
            <div class="container">
                <header class="header">
                    <h1>{titulo}</h1>
                    <div class="subtitle">
                        📊 {total_filas} grupos • 🀄 {caracteres_chinos} caracteres chinos
                    </div>
                </header>
    ...
    def generar_html_vacio(self):
        ...
        <body>
            <div class="header">
                <h1>🎌 Pronunciación Pinyin ⚡</h1>
                <p>Ingresa texto chino para ver la pronunciación</p>
            </div>
            
            <div style="text-align: center; padding: 50px; color: #6b7280;">
                <div style="font-size: 48px; margin-bottom: 20px;">🈳</div>
                <h2>Listo para mostrar Pinyin</h2>
                <p>La tabla aparecerá aquí cuando traduzcas texto chino</p>
                <p style="margin-top: 20px; font-size: 14px;">
                    ⚡ HTML ultra optimizado • 🚀 Renderizado instantáneo • 📱 Totalmente responsive
                </p>
            </div>
            {self.js_interactivo}
        </body>
...
class TraductorChino:
    ...
    def __init__(self):
        ...
        self.app.title("Traductor Chino Avanzado")
        ...
        # Detectar hardware y configurar automáticamente
        self.hardware_info = self.detectar_hardware()
        self.configurar_optimizaciones_hardware()
        
        # Caché persistente para traducciones
        ...
        # Variable para rastrear el idioma de destino actual
        self.idioma_destino_actual = 'es'  # Por defecto español
        
        # Variable para contenido completo (traducción + pinyin + caracteres)
        ...
        # Pool de hilos dinámico optimizado para cualquier CPU
        ...
        self.actualizacion_traduccion_en_progreso = False
        self.idioma_destino_pendiente = None
        self.ultimo_idioma_renderizado = self.idioma_destino_actual
        
        # Control de optimización de UI (ultra mejorado)
        ...
        # Configurar ventana principal con geometría adaptativa
        geometria = self.calcular_geometria_inicial()
        self.app.geometry(geometria)
        self.app.title("Traductor Chino → Español e Inglés con Pinyin [Optimizado Universal]")
        ...
        # Configurar tamaño mínimo adaptativo
        min_size = self.calcular_tamaño_minimo()
        ...
        # Configurar optimización periódica adaptativa
        ...
        # Guardar caché al cerrar
        self.app.protocol("WM_DELETE_WINDOW", self.on_closing)
        
        # Mostrar contenido inicial de forma asíncrona para evitar bloqueos
        self.app.after(100, self.mostrar_contenido_inicial)
    
    def traducir_grupo_seguro(self, texto_grupo, idioma_target='es'):
        """Traduce un grupo de caracteres de forma segura con caché y manejo de errores"""
        if not texto_grupo or len(texto_grupo) < 2:
            return None
            
        # Crear clave de caché
        cache_key = f"{texto_grupo}_{idioma_target}"
        
        # Verificar caché primero
        if cache_key in self.cache_traducciones_grupos:
            return self.cache_traducciones_grupos[cache_key]
        ...
        try:
            ...
            start_time = time.time()
            traductor = GoogleTranslator(source='zh-CN', target=idioma_target)
            resultado = traductor.translate(texto_grupo)
            ...
            if resultado and resultado.strip():
                # Guardar en caché solo si es exitosa
                self.cache_traducciones_grupos[cache_key] = resultado
                return resultado
    ...
    def obtener_marca_cpu(self):
        ...
        try:
            ...
            if 'intel' in processor:
                ...
                # Método alternativo usando WMI (Windows)
                try:
                    import subprocess
    ...
    def configurar_optimizaciones_hardware(self):
        ...
        try:
            ...
            # Configuraciones específicas por marca de CPU
            if hw['cpu_brand'] == 'AMD':
                # AMD suele beneficiarse de más hilos concurrentes
            ...
            if hw['is_low_end']:
                ...
                self.frecuencia_gc = 5000  # GC más frecuente
            elif hw['is_high_end']:
    ...
    def calcular_workers_optimos(self):
        ...
        try:
            ...
            # Límites basados en memoria disponible
    ...
    def calcular_update_interval(self):
        """Calcula el intervalo de actualización UI óptimo"""
        try:
            hw = self.hardware_info
    ...
    def calcular_limite_segmento(self):
        """Calcula el límite de segmento óptimo para traducción"""
        try:
            hw = self.hardware_info
            base_limite = 2000
            
            # Ajustar según memoria y CPU
    ...
    def calcular_geometria_inicial(self):
        """Calcula la geometría inicial de ventana según el hardware"""
        try:
            hw = self.hardware_info
            
            if hw['is_low_end']:
                return "1200x700"  # Más pequeño para hardware bajo
            elif hw['is_high_end']:
                return "1600x900"  # Más grande para hardware alto
            else:
                return "1400x800"  # Tamaño estándar
                
        except:
            return "1400x800"
    
    def calcular_tamaño_minimo(self):
        """Calcula el tamaño mínimo de ventana"""
        try:
            hw = self.hardware_info
    ...
    def calcular_intervalo_optimizacion(self):
        """Calcula el intervalo de optimización periódica"""
        try:
            hw = self.hardware_info
    ...
    def cargar_cache(self, tipo):
        ...
        try:
            ...
            if cache_file.exists():
                ...
                print(f"Caché {tipo} cargado: {len(cache_data)} elementos")
                return cache_data
        except Exception as e:
            print(f"Error cargando caché {tipo}: {e}")
    ...
    def guardar_cache(self, tipo, datos):
        ...
        try:
            ...
            with open(cache_file, 'wb') as f:
                pickle.dump(datos, f)
            print(f"Caché {tipo} guardado: {len(datos)} elementos")
        except Exception as e:
            print(f"Error guardando caché {tipo}: {e}")
    ...
    def on_window_resize_ultra_optimizado(self, event):
        """Manejo ultra optimizado de redimensionamiento con mínimo impacto"""
        ...
        # Delay adaptativo según hardware
    ...
    def ejecutar_ajuste_ultra_responsive(self):
        ...
        try:
            ...
            # Obtener dimensiones con verificación de cambio significativo
            ancho_actual = self.app.winfo_width()
            alto_actual = self.app.winfo_height()
            
            # Umbral adaptativo según hardware
            ...
            # Cálculo ultra eficiente de parámetros
            if ancho_actual < 700:
    ...
    def actualizar_fuentes_rapido(self, font_size):
        """Actualización rápida de fuentes con mínimo impacto"""
        try:
            if not hasattr(self, 'ultimo_font_size') or self.ultimo_font_size != font_size:
                ...
                # Update solo elementos críticos
    ...
    def actualizar_info_responsive_rapido(self, ancho):
        """Actualización rápida de info responsive"""
        try:
            if hasattr(self, 'info_pinyin'):
                if ancho < 700:
                    ...
                    texto = "💡 Pronunciación disponible"
    ...
    def crear_botones_responsive(self):
        ...
        # Botón de copiar eliminado según solicitud del usuario
        
        self.btn_exportar_pdf = customtkinter.CTkButton(
            self.frame_botones,
            text="📄 PDF",
            command=self.exportar_html_como_pdf,
            font=("Arial", 16, "bold"),  # Fuente más grande
            height=45,  # Más alto
            corner_radius=10,
            fg_color="#e74c3c",
        ...
        self.btn_exportar_pdf.grid(row=0, column=2, padx=5, pady=10, sticky="ew")
    ...
    def crear_seccion_pinyin_derecha(self):
        """Crea la sección de pinyin en el lado derecho con HTML viewer optimizado"""
        ...
        # Info responsive con indicador de tecnología
        tecnologia = "HTML⚡" if self.usar_html_viewer else "Tabla"
        self.info_pinyin = customtkinter.CTkLabel(
            header_pinyin,
            text=f"💡 {tecnologia} - Listo para mostrar pronunciación",
            font=("Arial", 12),
            text_color="gray"
        )
        ...
        # Crear viewer según disponibilidad
        ...
        # *** No llamar mostrar_contenido_inicial aquí para evitar bloqueos ***
        # Se llamará después de que termine la inicialización completa
    
    def crear_html_viewer(self):
        """Crea el viewer HTML optimizado sin conflictos de scroll"""
        try:
            # Frame normal como contenedor (sin scroll automático)
            ...
            # HTML Frame optimizado con scroll propio - con timeout
            if HTML_DISPONIBLE and HtmlFrameClass is not None:
                try:
                    # Crear con configuración mínima para evitar bloqueos
                    ...
                    print(f"Error inicializando HTML frame: {html_error}")
                    # Fallback a modo textbox
                    self.usar_html_viewer = False
                    self.crear_fallback_viewer()
            else:
                print("tkinterweb.HtmlFrame no disponible - usando fallback")
                self.usar_html_viewer = False
                self.crear_fallback_viewer()
    ...
    def crear_fallback_viewer(self):
        ...
        # Textbox para mostrar información
    ...
    def mostrar_contenido_inicial(self):
        """Muestra el contenido inicial según el modo activo"""
    ...
    def actualizar_contenido_pinyin_html(self, datos_tabla):
        ...
        try:
            ...
            html_contenido = self.generador_html.generar_html_optimizado(
                datos_tabla, 
                "Pronunciación Pinyin - Ultra Optimizado",
                contenido_completo,
            ...
            idioma_label = "Español" if getattr(self, 'idioma_destino_actual', 'es') == 'es' else "English"
            self.info_pinyin.configure(text=f"💡 HTML⚡ - {total_filas} filas cargadas · {idioma_label}")
            self.ultimo_idioma_renderizado = getattr(self, 'idioma_destino_actual', 'es')
    ...
    def abrir_html_en_navegador(self):
        ...
        try:
            ...
            self.info_pinyin.configure(text="💡 HTML⚡ - Abierto en navegador")
    ...
    def limpiar_html_viewer(self):
        ...
        if self.usar_html_viewer and hasattr(self, 'html_frame'):
            try:
                ...
                self.info_pinyin.configure(text="💡 HTML⚡ - Listo para mostrar pronunciación")
            except Exception as e:
    ...
    def actualizar_progreso(self, valor, texto=""):
        ...
        try:
            # Control de frecuencia de actualización para evitar lag
            ahora = time.time()
            ...
            if hasattr(self, 'progress_bar'):
                ...
                self.app.update_idletasks()  # Más eficiente que update()
        except Exception as e:
    ...
    # Funciones de copiado eliminadas según solicitud del usuario
    ...
    def optimizacion_periodica(self):
        """Optimización periódica inteligente adaptada al hardware"""
        try:
    ...
    def optimizar_cache_inteligente(self):
        """Optimiza cachés de manera inteligente según uso de memoria"""
        try:
            # Obtener uso actual de memoria
            memoria_actual = psutil.virtual_memory()
            porcentaje_uso = memoria_actual.percent
            
            # Ajustar límites de caché según memoria disponible
            ...
            # Limpiar cachés si exceden límites adaptativos
            self.gestionar_cache_traducciones()
            self.gestionar_cache_pinyin_periodico()
            
            print(f"Optimización caché: {porcentaje_uso}% memoria, límite: {self.limite_cache_mb}MB")
            
        except Exception as e:
            print(f"Error optimizando caché: {e}")
    
    def limpiar_cache_agresivo(self):
        """Limpieza agresiva de caché cuando la memoria es crítica"""
        try:
            # Reducir caché de traducciones a elementos esenciales
            ...
            gc.collect()
            print("Limpieza agresiva de caché ejecutada")
            
        except Exception as e:
            print(f"Error en limpieza agresiva: {e}")
    
    def gestionar_cache_traducciones(self):
        """Gestiona el caché de traducciones de manera inteligente"""
        try:
    ...
    def gestionar_cache_pinyin_periodico(self):
        """Gestiona el caché de pinyin de manera periódica"""
        try:
            limite_elementos = int(self.limite_cache_mb * 0.5)  # 50% del límite para pinyin
            ...
            if len(self.cache_pinyin) > limite_elementos:
                ...
                print(f"Caché Pinyin optimizado: {len(self.cache_pinyin)} elementos")
                
        except Exception as e:
            print(f"Error gestionando caché pinyin: {e}")
    ...
    def optimizar_memoria_sistema(self):
        ...
        try:
            ...
            if memoria.percent > 80 or (hasattr(self, 'ultima_gc') and 
                ...
                gc.collect()
                self.ultima_gc = time.time()
                print(f"Recolección de basura ejecutada - Memoria: {memoria.percent}%")
            
        except Exception as e:
            print(f"Error optimizando memoria: {e}")
    
    def ajustar_parametros_dinamicos(self):
        """Ajusta parámetros dinámicamente según el estado del sistema"""
        try:
            # Ajustar interval de updates según carga de CPU
    ...
    def traducir_a_espanol(self):
        ...
        # Establecer idioma de destino
        self.idioma_destino_actual = 'es'
        
        # Si ya hay una tabla Pinyin, solo actualizar las traducciones
        if hasattr(self, 'datos_tabla_actual') and self.datos_tabla_actual:
            self.actualizar_traducciones_tabla_pinyin()
            return
    ...
    def traducir_texto_optimizado(self, texto, idioma_destino):
        """Versión ultra optimizada de traducción con gestión adaptativa de recursos"""
        try:
            # Usar límite adaptativo calculado según hardware
            ...
            raise Exception(f"Error en traducción ultra optimizada: {str(e)}")
    
    def procesar_texto_por_chunks_adaptativos(self, texto, idioma_destino):
        """Procesa texto en chunks adaptativos según el hardware"""
        try:
            lineas = texto.split('\n')
            lineas_traducidas = []
            total_lineas = len(lineas)
            
            # Procesamiento en lotes según capacidad de hardware
    ...
    def calcular_batch_size(self, total_lineas):
        """Calcula el tamaño de lote óptimo según hardware"""
        try:
            hw = self.hardware_info
            
            # Base según número de workers
    ...
    def procesar_batch_lineas(self, lineas, idioma_destino):
        """Procesa un lote de líneas de manera optimizada"""
        resultado = []
    ...
    def calcular_pausa_adaptativa(self):
        ...
        try:
            ...
            if hw['is_high_end']:
                return 0.05  # Pausa mínima para hardware potente
            elif hw['is_low_end']:
                return 0.2   # Pausa mayor para hardware limitado
            else:
                return 0.1   # Pausa estándar
    ...
    def update_progreso_eficiente(self, valor, texto=""):
        ...
        try:
            ...
            # Usar queue para update no bloqueante si está disponible
    ...
    def actualizar_progreso_directo(self, valor, texto=""):
        """Update directo de progreso con mínimo bloqueo"""
        try:
            if hasattr(self, 'progress_bar'):
    ...
    def generar_pinyin_optimizado(self, texto_chino):
        """Versión ultra optimizada de generación de pinyin con recursos adaptativos"""
        try:
            ...
            # Verificar caché primero con sistema inteligente
            cache_key = f"pinyin_{hash(texto_chino)}"  # Hash para claves más eficientes
    ...
    def actualizar_contenido_completo(self, texto_chino, datos_tabla):
        """Actualiza la variable de contenido completo con toda la información"""
        try:
    ...
    def crear_markdown_completo(self, texto_original, datos_tabla):
        """Crea formato markdown completo con toda la información"""
        try:
            markdown = "# 📋 Traducción Completa con Pinyin\n\n"
            
            # Agregar traducción en inglés (texto principal)
            if self.traduccion_actual:
                markdown += f"## 🌐 Traducción al Inglés\n{self.traduccion_actual}\n\n"
            
            # Agregar texto original en chino
            if texto_original:
                markdown += f"## 🀄 Texto Original (Chino)\n{texto_original}\n\n"
            
            # Agregar tabla de pinyin con formato mejorado
            if datos_tabla:
                markdown += "## 📚 Tabla de Pronunciación Pinyin\n\n"
                ...
                # Procesar última fila si no terminó con separador
                if fila_pinyin_actual and fila_caracteres_actual:
                    filas_procesadas.append({
                        'pinyin': fila_pinyin_actual,
                        'caracteres': fila_caracteres_actual
            ...
            markdown += "\n---\n*Generado por Traductor de Chino Ultra Optimizado* ⚡"
            return markdown
    ...
    def contiene_caracteres_chinos_rapido(self, texto):
        """Verificación rápida de caracteres chinos"""
        try:
            # Verificación optimizada usando any() con generator
            return any('\u4e00' <= c <= '\u9fff' for c in texto[:100])  # Solo primeros 100 chars
        except:
            return False
    
    def crear_datos_pinyin_ultra_rapido(self, texto_completo):
        """Versión ultra rápida de creación de datos pinyin con batching"""
        try:
            # Calcular caracteres por línea adaptativos
            ...
            for idx_linea, linea in enumerate(lineas):
                if not linea.strip():
                    # Separador compacto en lugar de línea grande
                    datos.append(['⭐'])
                    continue
                
                # Procesar línea en chunks
                datos_linea = self.procesar_linea_pinyin_por_chunks(linea, batch_size)
                datos.extend(datos_linea)
                
                # Update progreso eficiente cada cierto número de líneas
                ...
                # Separador compacto entre líneas si no es la última
            ...
            print(f"Error creando datos pinyin ultra rápido: {e}")
            return []
    
    def calcular_caracteres_por_linea_adaptativos(self, texto):
        """Calcula caracteres por línea de manera adaptativa"""
        try:
            ...
            # Ajustar según cantidad de texto
    ...
    def procesar_linea_pinyin_por_chunks(self, linea, batch_size):
        """Procesa una línea de pinyin en chunks para mejor rendimiento"""
        try:
            ...
            print(f"Error procesando línea por chunks: {e}")
            return []
        ...
        try:
            ...
            for char in grupo:
                if '\u4e00' <= char <= '\u9fff':
                    ...
                    # Puntuación importante - preservar con emoji
                    emoji_map = {
                        '!': '❗', '?': '❓', '！': '❗', '？': '❓',
                        '.': '⭐', '。': '⭐', ',': '💫', '，': '💫',
                        ':': '🔸', '：': '🔸', ';': '🔹', '；': '🔹'
                    }
                    emoji = emoji_map.get(char, '📌')
                    fila_pinyin.append(f"{emoji}{char}")
                    fila_caracteres.append(char)
                elif char.strip():
                    # Otros caracteres no vacíos
                    fila_pinyin.append('·')
                    fila_caracteres.append(char)
                else:
                    # Espacios
    ...
    def gestionar_cache_inteligente(self, cache_key, datos):
        """Gestiona el caché de manera inteligente según memoria disponible"""
        try:
            # Calcular tamaño aproximado del caché
            cache_size_mb = len(self.cache_pinyin) * 0.1  # Estimación aproximada
            
            # Limpiar caché si excede el límite
            if cache_size_mb > self.limite_cache_mb:
                # Mantener solo los elementos más recientes
                elementos_mantener = int(self.limite_cache_mb * 10)  # Conversión inversa
            ...
            print(f"Error gestionando caché: {e}")
    
    def crear_tabla_pinyin_ultra_optimizada(self, datos_tabla):
        """Crea tabla de pinyin con optimización ultra usando HTML o CTkTable"""
        try:
            if not datos_tabla:
                return
    ...
    def crear_tabla_ctk_fallback(self, datos_tabla):
        ...
        try:
            ...
            # Actualizar info
            self.info_pinyin.configure(text=f"💡 Tabla - {filas} filas cargadas")
    ...
    def actualizar_traducciones_tabla_pinyin(self):
        """Actualiza traducciones existentes mostrando progreso y sin bloquear la UI"""
        try:
            if not hasattr(self, 'datos_tabla_actual') or not self.datos_tabla_actual:
                return

            idioma_objetivo = getattr(self, 'idioma_destino_actual', 'es')

            if getattr(self, 'actualizacion_traduccion_en_progreso', False):
                self.idioma_destino_pendiente = idioma_objetivo
                print(f"⏳ Actualización en curso, se programó el idioma pendiente: {idioma_objetivo}")
                return

            if hasattr(self, 'cache_traducciones_grupos'):
                elementos_cache = len(self.cache_traducciones_grupos)
                if elementos_cache:
                    print(f"🧹 Limpiando caché de traducciones de grupos ({elementos_cache} elementos)")
                self.cache_traducciones_grupos.clear()

            self.actualizacion_traduccion_en_progreso = True
            self.idioma_destino_pendiente = None

            nombre_idioma = "Español" if idioma_objetivo == 'es' else "English"
            mensaje_inicio = "Actualizando traducciones al español..." if idioma_objetivo == 'es' else "Updating translations to English..."

            self.mostrar_barra_progreso(mensaje_inicio)
            self.actualizar_progreso(5, "Preparando actualización...")

            def trabajo():
                idioma_trabajo = idioma_objetivo

                def update_step(valor, texto):
                    self.app.after(0, lambda v=valor, t=texto: self.actualizar_progreso(v, t))

                try:
                    update_step(15, "Analizando estructura de tabla...")

                    if self.usar_html_viewer and hasattr(self, 'html_frame'):
                        contenido_completo = getattr(self, 'contenido_completo_actual', None)
                        update_step(35, "Generando HTML optimizado...")
                        html_contenido = self.generador_html.generar_html_optimizado(
                            self.datos_tabla_actual,
                            "Pronunciación Pinyin - Ultra Optimizado",
                            contenido_completo,
                        ...
                        update_step(80, "Aplicando nuevo idioma en el visor...")

                        def cargar_html():
                            try:
                                self.html_frame.load_html(html_contenido)
                                total_filas = len(self.datos_tabla_actual) if self.datos_tabla_actual else 0
                                self.info_pinyin.configure(
                                    text=f"💡 HTML⚡ - {total_filas} filas · {nombre_idioma}"
                                )
                                self.ultimo_idioma_renderizado = idioma_trabajo
                                self.actualizar_progreso(95, "Casi listo...")
                            except Exception as e:
                                print(f"❌ Error aplicando HTML actualizado: {e}")
                        self.app.after(0, cargar_html)
                    else:
                        update_step(60, "Reconstruyendo tabla Pinyin...")
                        ...
                        def regenerar_tabla():
                            try:
                                ...
                                total_filas = len(self.datos_tabla_actual) if self.datos_tabla_actual else 0
                                self.info_pinyin.configure(
                                    text=f"📋 Tabla CTk - {total_filas} filas · {nombre_idioma}"
                                )
                                self.ultimo_idioma_renderizado = idioma_trabajo
                                self.actualizar_progreso(95, "Casi listo...")
                            except Exception as e:
                                print(f"❌ Error actualizando tabla CTk: {e}")
                        self.app.after(0, regenerar_tabla)

                except Exception as e:
                    print(f"❌ Error actualizando traducciones de tabla Pinyin: {e}")
                    self.app.after(0, lambda: self.mostrar_error(f"Error actualizando traducciones: {e}"))
                finally:
                    def finalizar():
                        self.actualizar_progreso(100, "Actualización completada")
                        self.actualizacion_traduccion_en_progreso = False
                        self.app.after(350, self.ocultar_barra_progreso)

                        pendiente = getattr(self, 'idioma_destino_pendiente', None)
                        if pendiente and pendiente != idioma_trabajo:
                            print(f"🔁 Procesando idioma pendiente: {pendiente}")
                            self.idioma_destino_pendiente = None
                            self.actualizar_traducciones_tabla_pinyin()

                    self.app.after(0, finalizar)

            threading.Thread(target=trabajo, daemon=True).start()

        except Exception as e:
            self.actualizacion_traduccion_en_progreso = False
            print(f"❌ Error general actualizando traducciones de tabla Pinyin: {e}")
            self.mostrar_error(f"Error general actualizando traducciones: {e}")
    ...
    def deshabilitar_botones(self):
        ...
        botones = [self.btn_traducir, self.btn_auto, self.btn_espanol, self.btn_ingles, self.btn_exportar_pdf]
        for btn in botones:
        ...
        try:
            ...
            if hasattr(self, 'info_pinyin'):
                ...
                if total_caracteres > 50:
                    mensaje_info = f"💡 {total_caracteres} caracteres procesados • {idioma_label} • Usa scroll para navegar"
                else:
                    mensaje_info = f"📋 Tabla Pinyin cargada • {idioma_label}"
                self.info_pinyin.configure(text=mensaje_info)
    ...
    def exportar_html_como_pdf(self):
        """Exporta el contenido HTML del viewer como PDF"""
        try:
            # Verificar que hay contenido para exportar
            if not hasattr(self, 'datos_tabla_actual') or not self.datos_tabla_actual:
                self.mostrar_error("No hay tabla de Pinyin para exportar")
                return
            ...
            try:
                ...
                self.mostrar_error(f"Error importando librerías: {e}")
                return
    ...
    def generar_html_para_pdf(self):
        ...
        try:
            ...
            total_filas = len(self.datos_tabla_actual) if getattr(self, 'datos_tabla_actual', None) else 0
            total_grupos = total_filas // 2
            texto_original = getattr(self, 'texto_chino_actual', 'N/A') or 'N/A'
            resumen_texto = texto_original[:100] + ('...' if len(texto_original) > 100 else '')

            info_exportacion = f"""
            <div class="export-info">
                <h3>Información de Exportación</h3>
                <p><strong>Fecha de exportación:</strong> {fecha_actual}</p>
                <p><strong>Generado por:</strong> Traductor de Chino Ultra Optimizado</p>
                <p><strong>Total de filas:</strong> {total_filas}</p>
                <p><strong>Grupos procesados:</strong> {total_grupos}</p>
                <p><strong>Texto original:</strong> {resumen_texto}</p>
            </div>
            """
            
            html_modificado = html_modificado.replace('</body>', f'{info_exportacion}</body>')
            
            return html_modificado
            
        except Exception as e:
            print(f"Error generando HTML para PDF: {e}")
            # Fallback básico
            return self.generador_html.generar_html_optimizado(
                self.datos_tabla_actual,
                "Tabla de Pronunciación Pinyin",
                None,
                self.idioma_destino_actual
            )
    ...
    def mostrar_mensaje_info(self, titulo, mensaje):
        ...
        try:
            ...
            # Centrar la ventana
            ventana_info.transient(self.app)
    ...
    def preguntar_si_no(self, pregunta):
        """Muestra un diálogo de sí/no"""
        try:
            from tkinter import messagebox
            return messagebox.askyesno("Confirmación", pregunta)
        except:
            return True  # Default a sí si hay error
